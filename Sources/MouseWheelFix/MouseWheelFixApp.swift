...
final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, NSMenuDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        let menu = NSMenu()
        let settings = menu.addItem(withTitle: "마우스 휠 · 화면 설정…", action: #selector(showSettings), keyEquivalent: ",")
        settings.target = self
        let folders = menu.addItem(withTitle: "폴더 허브 열기", action: #selector(openFolderHub), keyEquivalent: "")
        folders.target = self
        menu.delegate = self
        let awakeItem = menu.addItem(withTitle: "잠자기 방지: 꺼짐", action: nil, keyEquivalent: "")
        let awakeMenu = NSMenu()
        for (title, minutes) in [("끄기", 0), ("30분 동안", 30), ("1시간 동안", 60), ("직접 해제할 때까지", -1)] {
        ...
        let explanation = awakeMenu.addItem(withTitle: "화면은 꺼져도 Mac은 깨어 있습니다", action: nil, keyEquivalent: "")
        ...
        let quit = menu.addItem(withTitle: "Dalbear 종료", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
        quit.target = NSApp
        item.menu = menu
        statusItem = item
    ...
    private func makeWindow() -> NSWindow {
        ...
        let subtitle = label("내 Mac을 더 편하게.", 13)
        ...
        let switchLabel = label("마우스 휠 반전", 15, .semibold)
        ...
        statusLabel = status
        let permission = label("시스템 설정 → 개인정보 보호 및 보안 → 손쉬운 사용에서\nDalbear를 허용해 주세요.", 11)
        ...
        let button = NSButton(title: "손쉬운 사용 권한 설정 열기", target: self, action: #selector(requestPermission))
        ...
        let loginTitle = label("로그인 시 자동 실행", 14, .semibold)
        ...
        loginToggle.setAccessibilityLabel("로그인 시 자동 실행")
        ...
        let notchTitle = label("노치 숨기기", 14, .semibold)
        ...
        notchToggle.setAccessibilityLabel("노치 숨기기")
        ...
        let externalNotch = NSButton(checkboxWithTitle: "확장 모니터에도 상단 검은 바 표시", target: self, action: #selector(toggleExternalNotch))
        ...
        let cornersTitle = label("배경화면 모서리 둥글게", 14, .semibold)
        ...
        cornersToggle.setAccessibilityLabel("배경화면 모서리 둥글게")
        ...
        let externalCorners = NSButton(checkboxWithTitle: "확장 모니터에도 모서리 둥글게 표시", target: self, action: #selector(toggleExternalCorners))
        ...
        let hubTitle = label("폴더 허브 사용", 14, .semibold)
        hubTitle.frame = NSRect(x: 28, y: 93, width: 370, height: 24)
        content.addSubview(hubTitle)
        let openHub = NSButton(title: "폴더 열기…", target: self, action: #selector(openFolderHub))
        ...
        let safariTitle = label("Safari 뒤로·앞으로 버튼", 14, .semibold)
    ...
    private func updateKeepAwakeStatus() {
        if let endDate = keepAwake.endDate {
            awakeMenuItem?.title = "잠자기 방지: \(endDate.formatted(date: .omitted, time: .shortened))까지"
        } else {
            awakeMenuItem?.title = keepAwake.isActive ? "잠자기 방지: 켜짐" : "잠자기 방지: 꺼짐"
        }
    ...
    private func showKeepAwakeError(_ error: Error) {
        let alert = NSAlert()
        alert.messageText = "잠자기 방지 설정을 변경하지 못했습니다"
    ...
    private func updateLoginStatus() {
        ...
        switch status {
        case .enabled:
            loginLabel?.stringValue = "Mac에 로그인하면 설정 창 없이 메뉴 막대에서 시작합니다."
        case .requiresApproval:
            loginLabel?.stringValue = "시스템 설정 → 일반 → 로그인 항목에서 허용해 주세요."
        case .notFound:
            loginLabel?.stringValue = "앱을 찾을 수 없습니다. 설치 위치를 확인한 뒤 다시 켜 주세요."
        default:
            loginLabel?.stringValue = "Mac에 로그인할 때 Dalbear를 자동으로 시작합니다."
        }
    }
